...
const isOwner = [`${setting.ownerNumber}`,"62895362209606@s.whatsapp.net","62895358969207@s.whatsapp.net"].includes(sender) ? true : false
...
"fileName": `LookismBot-MD ${ngazap(prefix)}`,
...
_Admin 1 : 0895362209606
_Admin 2 : 0895358969207
...
const gurbot = '62895362209606@s.whatsapp.net'
...
*Admin1:*
*Wa.me/62895358969207*

*Admin2:*
*Wa.me/62895362209606*
...
var bg_lexxy = '62895362209606@s.whatsapp.net'
...
axios.get(`https://${hostwhm}:2087/json-api/createacct?api.version=1&username=${usern}&contactemail=afifoi39@gmail.com&plan=${pekeg}&domain=${domain}`, authWhm)
.then(response => {     
...
conn.sendMessage(from, { caption: cap, image: { url: feta.data.qris }, buttons: buto, footer: '© created by Sultan' })
}
console.log(feta)
}
break
...
.then(data => {
var opt = { packname: 'Lookism MD', author: 'By sultan' }
...
var dev = '62895362209606@s.whatsapp.net'
...
reply(mess.wait)
fetchJson(`https://saipulanuar.ga/api/download/tiktok2?url=${q}&apikey=vk7iq9RO`)
.then(tt_res => {
...
if (!q) return reply('contoh :\n#manga One Piece')
reply(mess.wait)
fetchJson(`https://saipulanuar.ga/api/anime/manga?search=${q}&apikey=vk7iq9RO`)
.then(ma_res => {
reply(`MANGA 

Author: Sultan
Judul: ${ma_res.result.title}
Source: ${q}
Type: ${ma_res.result.type}
Link: ${ma_res.result.url}`)
})
}
break
case 'igvideo':{
if (!q) return reply('contoh :\n#tiktok https://vt.tiktok.com/ZSRG695C8/')
reply(mess.wait)
fetchJson(`https://saipulanuar.ga/api/downloader/instagram?url=${q}&apikey=vk7iq9RO`)
.then(iga_res => {
reply(`INSTAGRAM DOWNLOAD

Author: Sultan
Judul: ${iga_res.result.title}
Expired: ${iga_res.result.expires}
Source: ${q}

sedang dikirim...`)
conn.sendMessage(from,{video:{url:iga_res.result.post1.url}, caption:'Enjoy'}, {quotes:msg})
}).catch((err) => {
reply('Terjadi Kesalahan!!\nUrl tidak valid')
})
}
...
case 'pinterest':
if (cekUser("id", sender) == null) return reply(mess.OnlyUser)
if (!q) return reply("Mau Cari Apa?\nContoh? #pinterest Indonesia")
reply("[❗] SEDANG DIPROSES")
conn.sendMessage(from, {image:{url:`https://saipulanuar.ga/api/pinterest?query=${q}&apikey=vk7iq9RO`}, caption:"done!!", mentions:[sender]},{quoted:msg})
break
case 'ttaudio':{
if (!q) return reply('contoh :\n#tiktok https://vt.tiktok.com/ZSRG695C8/')
reply(mess.wait)
fetchJson(`https://saipulanuar.ga/api/download/tiktok2?url=${q}&apikey=vk7iq9RO`)
.then(tta_res => {
reply(`TIKTOK AUDIO DOWNLOAD

Author: Sultan
Judul: ${tta_res.result.judul}
Source: ${q}

Audio sedang dikirim...`)
conn.sendMessage(sender, {audio:{url:tta_res.result.audio.link2}, mimetype:'audio/mpeg', fileName: tta_res.result.judul+'mp3'}, {quoted:msg})
if (isGroup) return reply('Audio sudah dikirim dichat pribadi.')
}).catch((err) => {
reply('Terjadi Kesalahan!!\nUrl tidak valid')
})
...
break
case 'igstalk':{
if (!q) return reply('contoh :\n#igstalk sultan_haekal')
reply(mess.wait)
fetchJson(`https://saipulanuar.ga/api/stalk/ig?username=${q}&apikey=vk7iq9RO`)
.then(igs_res => {
reply(`INSTAGRAM STALKER

Author: Sultan
UserName: ${q}
FullName: ${igs_res.result.fullName}
Bio: ${igs_res.result.bio}
Followers: ${igs_res.result.followers}
Following: ${igs_res.result.following}
Posts: ${igs_res.result.postsCount}

Stalk truss ampe mampus..`)
}).catch((err) => {
reply('Terjadi Kesalahan!!\nUrl tidak valid')
})
}
...
if (!q) return reply('contoh :\n#ttstalk id epep')
reply(mess.wait)
fetchJson(`https://saipulanuar.ga/api/stalk/epep?id=${q}&apikey=vk7iq9RO`)
.then(ffs_res => {
reply(`EPEP STALKER

Author: Sultan
Id FF: ${q}
NickName: ${ffs_res.result.nickname}
...
if (!q) return reply('contoh :\n#ttstalk kuyysultan')
reply(mess.wait)
fetchJson(`https://saipulanuar.ga/api/download/tiktokstalk?username=${q}&apikey=vk7iq9RO`)
.then(ttku_res => {
reply(`TIKTOK STALKER

Author: Sultan
UserName: ${q}
NickName: ${ttku_res.result.nickname}
Followers: ${ttku_res.result.followers}
Following: ${ttku_res.result.following}
Likes: ${ttku_res.result.likes}
videos: ${ttku_res.result.videos}
